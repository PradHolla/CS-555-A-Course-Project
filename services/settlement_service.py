...
class SettlementService:
    ...
    def get_debt_between_users(payer_email: str, recipient_email: str) -> float:
        ...
        # Calculate balances using existing service logic
        ...
        settlements_paid = db.session.query(db.func.sum(Settlement.amount)).filter(
            Settlement.payer_id == payer.id,
            Settlement.recipient_id == recipient.id
        ).scalar() or 0.0
    ...
    @staticmethod
    def create_settlement(payer_id: int, recipient_id: int, amount: float, note: str = None) -> tuple:
        """
