...
from models import Expense, User, UserGroupPoints
...
class PointsService:
    ...
    def get_or_create_user_group_points(user_id, group_id):
        ...
        """
        points_record = UserGroupPoints.query.filter_by(
            user_id=user_id, group_id=group_id
        ).first()
        
        if not points_record:
            points_record = UserGroupPoints(
                user_id=user_id,
                group_id=group_id,
                points=0
            )
        ...
        try:
            ...
            for expense in expenses:
                if PointsService.should_award_points(expense):
                    ...
                    if payer_user_id:
                        user_points[payer_user_id] = user_points.get(payer_user_id, 0) + PointsService.POINTS_PER_EXPENSE
                    else:
                        # Debug: log if we can't find the payer
                        print(f"Warning: Could not find user_id for payer '{payer_identifier}' in group {group_id}")
