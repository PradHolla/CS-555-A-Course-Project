...
def test_init_db_adds_receipt_columns(tmp_path):
    """If an existing sqlite DB misses receipt columns, init_db adds them."""
    db_path = tmp_path / "legacy.db"
    db_file = str(db_path)

    # Create an SQLite DB with minimal legacy tables (no receipt columns)
    conn = sqlite3.connect(db_file)
    cur = conn.cursor()

    # Create a legacy expense table (without receipt columns)
    cur.execute(
        """
    ...
    # Verify receipt columns exist now
    conn = sqlite3.connect(db_file)
    cur = conn.cursor()

    cur.execute("PRAGMA table_info('expense')")
    expense_cols = [row[1] for row in cur.fetchall()]
    assert "receipt_image" in expense_cols
    assert "receipt_uploaded_by" in expense_cols
    assert "receipt_uploaded_at" in expense_cols
